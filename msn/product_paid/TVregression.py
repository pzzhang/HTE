...
class TVregression(object):
    ...
    def get_u0(self, u):
        return self.c0 - np.dot(self.b0, u)
    ...
    def output_Bb(self, printb=False):
        ...
        if abs(np.linalg.norm(self.b - np.dot(u, np.dot(u.T, self.b)))) > 1e-14 * np.linalg.norm(self.b):
            ...
            print "Relative error in B and b: ", abs(np.linalg.norm(self.b - np.dot(u, np.dot(u.T, self.b))))/np.linalg.norm(self.b)
        # least norm least square solution
    ...
    def get_Bbc(self):
        ...
        for d in range(self.D):
            ...
            self.b0[gind] = ama_temp/self.msum
        ...
        for pairid, pair in enumerate(self.pairs):
            ...
            self.b0[ginpair] = ama_temp/self.msum
    ...
    def print_effect(self, u, lam, stepid, str_method):
        ...
        il1 = np.tril_indices(self.nt1, -1)
        effect[il1] = u0
        # diagonal is the first order effect, and uppper triangular part is the second order effect
        labels = []
        labelsx = []
        cross_ind = 0
        for d in range(self.D):
        ...
        plt.title("TV-regression: Lambda=%1.2e,u0=%1.2e" % (lam, u0))
        # plt.show()
