...
class VectorFussedLasso(object):
    ...
    def lasso_path(self, lam_min, lam_max, nstep=100, tol=1e-8):
        ...
        for i, lam in enumerate(lam_list):
            ...
            if not(is_changed.all()):
                ...
                self.print_effect_single(X_ls, lam, i, "LassoPath", xlabel=self.dates)
    ...
    def print_effect_single(self, u, lam, stepid, str_method, xlabel):
        # plot computed result
        fig, ax = plt.subplots()
        plt.plot(range(self.T), u, 'r', linewidth=2.0, label="fit")
        plt.errorbar(range(self.T), self.g, 1.96/np.sqrt(self.m*self.N), marker='^', ecolor='g', label='data')
        plt.xlim([-1, self.T])
        ax.set_xticks(np.arange(self.T), minor=False)
        ax.set_xticklabels(xlabel, minor=False, rotation=45)
        plt.legend(loc='best')
