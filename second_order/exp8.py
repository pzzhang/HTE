...
u_exact += u2_exact[1][:, np.newaxis, :, np.newaxis]
m *= Ns
...
chambo2_new = Chambolle2_new(g, m)
...
chambo2_new.get_K()
chambo2_new.set_w(Ns=10000)

# test lasso paht
lam_min = 100
lam_max = 200
u_lass_path, noise_lasso_path = chambo2_new.lasso_path(lam_min=lam_min, lam_max=lam_max, nstep=2)

# test Bregman iteration
lam = 100
u_bregman, noise_bregman = chambo2_new.bregman_iteration(lam=lam, nstep=2)
...
energy2, u12, u22 = chambo2_new.P_PDALG1(Lambda=Lambda, n_it=n_it)
...
# plt.show()
plt.savefig('figure/exp8_2/EnergyDecay_Lambda = %1.2e_n_it = %1.2e.png' % (Lambda, n_it))
plt.close(fig)
...
# plt.show()
plt.savefig('figure/exp8_2/FirstOrder_Lambda = %1.2e_n_it = %1.2e.png' % (Lambda, n_it))
plt.close(fig)
...
# plt.show()
plt.savefig('figure/exp8_2/GroundTruth.png')
plt.close(fig)
...
# plt.show()
plt.savefig('figure/exp8_2/LeastSquare.png')
plt.close(fig)
...
# plt.show()
plt.savefig('figure/exp8_2/SecondOrder_Lambda = %1.2e_n_it = %1.2e.png' % (Lambda, n_it))
plt.close(fig)