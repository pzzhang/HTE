...
class Chambolle2_new(object):
    ...
    def get_K(self):
        ...
        for pairid, pair in enumerate(self.pairs):
            ...
            for i in range(n1):
                ...
                self.A2[pairid][i * n0:(i + 1) * n0, i * n0:(i + 1) * n0] = self.A1[pair[0]]
            for j in range(n0):
                self.K2[pairid][(j*T1+n1*T0):((j+1)*T1+n1*T0), j::n0] = self.K1[pair[1]]
                self.A2[pairid][j::n0, j::n0] = self.A1[pair[1]]
        # total K
    ...
    def lasso_path(self, lam_min, lam_max, nstep=100, tol=1e-8):
        ...
        lam_list = 1.0/np.linspace(1.0/lam_max, 1.0/lam_min, nstep)
        ...
        for i, lam in enumerate(lam_list):
            ...
            u_list[i] = 1.0 * x
            # noise_energy = 0.5 * np.dot(x, np.dot(self.B, x)) - np.dot(x, self.b) + 0.5 * self.c
            if not ((np.abs(x) > 2*tol) == (np.abs(x_old) > 2*tol)):
                # support changes
    ...
    def bregman_iteration(self, lam, nstep = 100):
        # settings of the P_PDALG
        theta = 1.0
        tinv, sigmainv = self.get_tinv_sigmainv()
        # initial guess for the very beginning
        ...
        for i in range(nstep):
            ...
            if not((np.abs(x) > 2*tol) == (np.abs(x_old) > 2*tol)):
                ...
                print "Number of non-zero groups = ", num_groups
                print "num_iter, Noise_Energy", num_iter, noise_energy
                self.print_effect(x, lam, i, "BregmanIteration")
                uls_list.append(1.0*x_ls)
                noise_energy_list[i] = noise_energy
    ...
    def blockLS(self, x, tol):
        ...
        for termid, termA in enumerate(self.A):
            ...
            uloc = grouping(len(xloc), termA, tol)
            utrans.append(uloc)
    ...
    def print_effect(self, u, lam, stepid, str):
        # postprocessing
        u1 = [None] * self.D
        for d in range(self.D):
        ...
        for d in range(self.D):
            ...
            labels += ["X%d:%d" % (d + 1, i + 1) for i in range(self.n[d])]
            for f in range(d + 1, self.D):
                ginf = range(self.nind1[f], self.nind1[f + 1])
        ...
        # plt.show()
        plt.savefig('figure/'+str+'/Step%d_Lambda%1.2e.png' % (stepid, lam))
        plt.close(fig)
