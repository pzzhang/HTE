...
class Chambolle(object):
    ...
    def apply_K(self, x, Y):
        ...
        for d in range(self.D):
            ...
            for k in range(self.n[d]):
                Y[d][k][self.A[d][k]] = xd[self.A[d][k]] - xd[k]

    def apply_Kstar(self, x, Y):
        x[0] = 0
        for d in range(self.D):
            # add the weights to every row
            Y[d] = Y[d]/self.nlevel[d][:, np.newaxis]
            # Kstar*Y is flux in - fulx out
            x[self.nind1[d]:self.nind1[d+1]] = np.sum(Y[d], axis=0) - np.sum(Y[d], axis=1)
    ...
    def prox_Fstar(self, Y, sigma):
        for d in range(self.D):
            Y[d][self.A[d]] = proj_infty(Y[d][self.A[d]], self.w[d])

    def TV(self, Y):
        tv = 0.
        for d in range(self.D):
            tv += self.w[d]*np.sum(np.mean(np.abs(Y[d]), axis=1))
        return tv


def proj_infty(x, w):
    for i, elem in enumerate(x):
        x[i] = elem/max(1, abs(elem)/w)
    return x
