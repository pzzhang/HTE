...
u1_exact[1] = np.array([0.015,0,0,0,0])
u0_exact += np.mean(u1_exact[1])
u1_exact[1] -= np.mean(u1_exact[1])
# add noise
Ns = 10000
mm = [0.05*np.ones(20), 0.2*np.ones(5), np.array([0.1, 0.4, 0.3, 0.2])]
m = mm[0]
...
# A[0] = np.ones((nn[0], nn[0]), dtype=bool)
# A[0][range(0,nn[0]),range(0,nn[0])] = False
# chambo.set_graph(A)
chambo.set_w(w_type=3)
...
plt.show()
# plt.savefig('figure/exp2/Lambda = %1.2e_n_it = %1.2e.png' % (Lambda, n_it))
# plt.close(fig)
