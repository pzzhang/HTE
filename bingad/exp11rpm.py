...
# load data
df = pd.read_csv("data/volume2.csv", index_col=0).replace([np.inf, -np.inf], np.nan)
...
values = df.values
nn = [5, 2, 13, 7]
logdelta = np.array(values[:, 9].reshape(tuple(nn), order='C'), dtype=float)
mlogdelta = np.array(values[:, 11].reshape(tuple(nn), order='C'), dtype=float)
...
# test pure lasso
u_lasso_path, noise_lasso_path, uls_lasso_path = chambo2.lasso_path(lam_min=10, lam_max=400, nstep=400)

# # test Bregman iteration
# lam = 1.0
# u_bregman, noise_bregman, uls_bregman = chambo2.bregman_iteration(lam=lam, nstep=500)