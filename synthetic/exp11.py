...
effect_exact[il1] = u0_exact
u0_ls = chambo2.get_u0(chambo2.uls)
effect_ls[il1] = u0_ls
# the first and second order effect
labels = []
labelsx = []
cross_ind = 0
...
plt.title("Ground Truth, u0=%1.2e" % u0_exact)
# plt.show()
plt.savefig('figure/GroundTruth.png')
plt.close(fig)
...
print "Initial noise energy:", 0.5*chambo2.c

# print the very first step
chambo2.print_effect(u=np.zeros(chambo2.nT), lam=0, stepid=-1, str_method="LassoPath")

# # test pure lasso
# chambo2.set_w_lasso(Ns=10000)
# u_lars_path, noise_lars_path, uls_lars_path = chambo2.lars_path()
...
lam_min = 20
lam_max = 300
u_lasso_path, noise_lasso_path, uls_lasso_path = chambo2.lasso_path(lam_min=lam_min, lam_max=lam_max, nstep=400)
