...
class Chambolle(object):
    ...
    def get_ama_amg(self):
        ...
        # least square solution with standard regularization
        self.lssol = np.linalg.solve(self.ama, self.amg)
    ...
    def apply_F(self, Y):
        tv = 0.
        for d in range(self.D):
            tv += np.sum(np.abs(Y[d][self.A[d]])*(self.w[d][self.A[d]]))
        return tv
    ...
    def apply_K(self, x, Y):
        ...
        for d in range(self.D):
            ...
            # the appended 0 is due to the L1 penalty on x
            Y[d] = self.w[d]*(np.append(xd, 0.0) - xd[:, np.newaxis])
    ...
    def apply_Kstar(self, x, Y):
        ...
        for d in range(self.D):
            ...
            Ytemp = self.w[d]*Y[d]
            x[self.nind1[d]:self.nind1[d+1]] = np.sum(Ytemp[:, 0:self.n[d]], axis=0) - np.sum(Ytemp, axis=1)

    def get_tinv_sigmainv(self):
        tinv = np.zeros(self.nt1)
        sigmainv = [2.0*self.w[d]*self.A[d] for d in range(self.D)]
        for d in range(self.D):
            # alpha = 1
            tinv[self.nind1[d]:self.nind1[d + 1]] = np.sum(self.w[d][:, 0:self.n[d]], axis=0) + np.sum(self.w[d], axis=1)
            sigmainv[d][:, self.n[d]] = self.w[d][:, self.n[d]]
    ...
    def P_PDALG1(self, Lambda, n_it, return_energy=True):
        ...
        self.Gamma, self.U = np.linalg.eigh(Lambda*self.ama0 + np.diag(tinv))
        ...
        x_tilde = 1.0*x
        # dual variables
        Y = [np.zeros((self.n[d], self.n[d]+1)) for d in range(self.D)]
        Y_tilde = [np.zeros((self.n[d], self.n[d]+1)) for d in range(self.D)]
        ...
        for k in range(n_it):
            ...
            for d in range(self.D):
                Y[d][self.A[d]] += Y_tilde[d][self.A[d]]/sigmainv[d][self.A[d]]
            self.prox_Fstar(Y, sigmainv)
            # update primal variable
            x_old = 1.0*x
            self.apply_Kstar(x_tilde, Y)
            x = Lambda*self.amg + x_old*tinv - x_tilde
            # self.prox_G(x, tau)
            ...
            if return_energy:
                ...
                en[k] = energy
                if k % 10 == 0:
                    print("[%d] : energy %e \t fidelity %e \t penalty %e \t TV %e" % (k, energy, fidelity, penalty, tv))
